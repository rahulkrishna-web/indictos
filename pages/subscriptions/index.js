...
import { getFirestore, collection, onSnapshot,query, orderBy,where, QuerySnapshot, getDocs , doc, getDoc } from "firebase/firestore";
...
export default function Subscriptions() {
  ...
  useEffect(()=>{
    if(auth.currentUser){
      ...
      const q = query(subscriptionsRef, where("user", "==", auth.currentUser.uid))
  ...
  return (
    <div className={styles.container}>
      ...
      {auth.currentUser && (
        <div>
          Logged in 
          {subscriptions && subscriptions.map((m, index)=>(
          <div key={index}>
            <Paper sx={{p: 2, mb: 2}}>
            {m.id}
            </Paper>
